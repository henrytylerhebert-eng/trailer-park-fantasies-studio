...
def build_blocks(chapters_dir: Path, art_dir: Path, embed):
    """Turn chapter markdown into an ordered list of Blocks, page-by-page."""
    blocks = []
    ...
    for f in files:
        ...
        # Strip trailing continuity notes so they don't render on the final page
        notes_match = re.search(r'(?:^|\n)(?:—|--)\s*continuity notes\s*(?:—|--)', text, re.I)
        if notes_match:
        ...
        # Split text by PAGE N markers
        page_splits = re.split(r'(?:^|\n)(?:##\s*)?PAGE\s+(\d+)\b', text, flags=re.I)
        
        # Split index 0 is optional intro text of the chapter
        intro_text = page_splits[0].strip()
        if intro_text:
            # Check if this is just duplicating the chapter title
            if not (title and title.lower() in intro_text.lower() and len(intro_text) < len(title) + 20):
                blocks.append(Block(md_to_html(intro_text), weight=40, kind="prose"))
                
        # Iterate over pages (page_splits contains groups of [p_num, p_content])
        for i in range(1, len(page_splits), 2):
            p_num = int(page_splits[i])
            p_content = page_splits[i+1].strip()
            if not p_content:
                continue
                
            # Extract panel art [ART: ...] if present
            art_match = re.search(r'\[ART:\s*(.*?)\]', p_content, re.DOTALL | re.I)
            art_dir_text = ""
            if art_match:
                art_dir_text = art_match.group(1).strip()
                p_content = p_content.replace(art_match.group(0), "").strip()
                
            # Format page content
            page_html = ""
            if art_dir_text:
                page_html += f'<div class="panel-art-direction" data-prompt="{html.escape(art_dir_text)}">\n'
                page_html += f'  <span class="panel-art-badge">Panel Art Direction</span>\n'
                page_html += f'  <p class="panel-art-desc"><em>{html.escape(art_dir_text)}</em></p>\n'
                page_html += f'</div>\n'
                
            page_html += md_to_html(p_content)
            
            # Calculate word count for budgeting/weight
            words = len(re.findall(r"\w+", p_content))
            blocks.append(Block(page_html, max(words, 20), kind="prose", hard_break=True))
...
def populate(template_path, out_path, chapters_dir, art_dir, page_art_dir, embed,
             cover_path=None, intro_path=None):
    ...
    # Intro / context page — rendered from a source file every run, so a re-populate
    # can NEVER silently lose it (it lives in source control, not a hand-edit).
    if intro_path and intro_path.exists():
        raw = intro_path.read_text(encoding="utf-8")
        inner = raw if intro_path.suffix.lower() in (".html", ".htm") else md_to_html(raw)
        blocks.insert(0, Block(f'<div class="intro-page">{inner}</div>',
                               weight=WORDS_PER_PAGE, kind="intro", hard_break=True))
        print(f"  injected intro page from {intro_path.name}")

    # Cover: explicit --cover, else art/cover.png, else art/key_art.png (the poster).
    cover = cover_path if (cover_path and cover_path.exists()) else None
    if cover is None:
        for cand in (art_dir / "cover.png", art_dir / "key_art.png"):
            if cand.exists():
                cover = cand; break
    if cover:
        blocks.insert(0, Block(img_tag(cover, embed, cls="cover", alt="cover"),
                               weight=WORDS_PER_PAGE, kind="art", hard_break=True))
        print(f"  using cover: {cover.name}")
...
<style>
  ...
  * {{ box-sizing:border-box; }}
  ...
  .intro-page {{ font-size:16px; line-height:1.75; }}
  .intro-page h1, .intro-page h2 {{ text-align:center; }}
...
def main():
    ...
    ap.add_argument("--cover", type=Path, default=None,
                    help="cover image (default: art/cover.png, then art/key_art.png)")
    ap.add_argument("--intro", type=Path, default=None,
                    help="intro/context page source (.md or .html), injected as front matter")
    ...
    populate(a.template, a.out, a.chapters, a.art, a.page_art, embed,
             cover_path=a.cover, intro_path=a.intro)
